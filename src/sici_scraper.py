...
from .config import BASE_URL, HEADLESS, OUTPUT_JSON, CLICK_TIMEOUT, ROUND_TIMEOUT
...
class SiciSmsScraper:
    ...
    def _extract_node_info(self) -> dict:
        ...
        try:
            # 1. Extrair DECRETO
            decree_match = self.page.evaluate("""
                () => {
            ...
            """)
            if decree_match:
                info["decreto"] = decree_match
            
            # 2. Extrair informações de TABELAS (Titular, Cargo, Endereço, etc)
            table_rows = self.page.query_selector_all("table tr, div.info-row, div[class*='row']")
            ...
            for row in table_rows:
                try:
                    ...
                    if "Comunicaçõe" in text_content or "Telefone" in text_content or "E-mail" in text_content:
                        current_section = "comunicacoes"
                    elif "Endereço" in text_content or "Bairro" in text_content:
                        current_section = "endereco"
                    elif "Titular" in text_content or "Cargo" in text_content:
                        current_section = "geral"
                    
                    # Tentar extrair pares rótulo:valor
                    if len(cells) >= 2:
                        # Formato: [rótulo] [valor]
                        label = cells[0].text_content().strip().rstrip(":")
                        value = cells[1].text_content().strip() if len(cells) > 1 else ""
                        
                        if label and value and len(label) > 2:
                            if current_section == "comunicacoes":
                                info["comunicacoes"].append({
                                    "tipo": label,
                                    "valor": value
                                })
                            elif current_section == "endereco":
                                info["endereco"][label] = value
                            else:
                                info["geral"][label] = value
                    else:
                        # Formato de linha única com conteúdo
                        if len(text_content) > 10:
                            if current_section == "comunicacoes":
                                # Tentar separar rótulo e valor
                                ...
                                parts = text_content.split(":", 1)
                                if len(parts) == 2:
                                    info["endereco"][parts[0].strip()] = parts[1].strip()
                            else:
                                info["geral"][f"campo_{len(info['geral'])}"] = text_content
                
                except Exception:
                    continue
            
            # 3. Se não encontrou dados estruturados, extrair texto bruto
            if not info["geral"] and not info["endereco"] and not info["comunicacoes"]:
                content = self.page.evaluate("() => document.body.innerText")
                lines = content.split('\n')
                
                # Extrair linhas relevantes (pular header/footer)
                relevant_lines = []
                for line in lines[3:30]:  # Pular primeiras 3 linhas, pegar até 30
                    line = line.strip()
                    if line and len(line) > 3:
                        relevant_lines.append(line)
                
                info["texto_extraido"] = " | ".join(relevant_lines[:15])
